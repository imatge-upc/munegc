...
class VPooling(torch.nn.Module):
    def __init__(self, pool_rad, aggr='max'):
        super(VPooling, self).__init__()
    ...
    def forward(self, data):
        cluster = nn_geometric.voxel_grid(data.pos, data.batch, self.pool_rad, start=data.pos.min(
            dim=0)[0] - self.pool_rad * 0.5, end=data.pos.max(dim=0)[0] + self.pool_rad * 0.5)
...
class VGraphPooling(torch.nn.Module):
    def __init__(self, pool_rad, aggr='max',
